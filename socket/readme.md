...
- **参数说明**
	...
	- **inode**，待关闭套接字的 inode 节点
	- **flip**，待关闭的套接字文件的描述符

- **工作流程**
	
0. [sys_socket.c/sock_close( )](./sys_socket.c)中注释了源码
...
0. [sys_socket.c/sock_release( )](./sys_socket.c)中注释了源码
...
0. [sys_socket.c/inet_release( )](./sys_socket.c)中注释了源码
1. 获取套接字的传输控制块指针 **sk**
...
- **参数说明**
	...
	- **sk**，想要关闭的套接字的传输控制块
	- **timeout**，正在关闭控制块之间可以发送剩余数据的事件
	
- **工作流程**

0. [sys_socket.c/tcp_close( )](./sys_socket.c)中注释了源码
...
6. 再给对端发送 RST 或者 FIN 后，等待套接字的关闭，知道套接字的状态 FIN_WAIT_1、CLOSING、LAST_ACK或者等待超时
...
13. 此时套接字状态时 TCP_CLOSE 则需释放传输控制块资源
